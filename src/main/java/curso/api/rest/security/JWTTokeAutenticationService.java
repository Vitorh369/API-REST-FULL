...
// CLASS QUE GERA O NOSSO TOKEN E TAMBEM VALIDA O TOKEN ENVIADO
@Service
@Component
public class JWTTokeAutenticationService {
	...
	public void addAuthentication(HttpServletResponse response , String username) throws IOException {
		...
		/*LIBERANDO RESPOTA PARA PORTA DO PROJETO ANGULAR*/
		liberacaoCors(response);
		
		/*atualizando token no banco de dados*/
		ApplicationContextLoad.getApplicationContext()
        .getBean(UsuarioRepository.class).atulizaTokenUser(JWT, username);
	...
	try {	
		...
		} /*FIM DA CONNDIÇÃO DO TOKEN*/
	}catch (io.jsonwebtoken.ExpiredJwtException e) {
		try {
			response.getOutputStream().println("Seu TOKEN esta expirado, faça login ou informe um novo TOKEN");
		} catch (IOException e1) {}
	}
		/*LIBERANDO Cors PARA PROJETO ANGULAR OU PARA POSTMAN*/
		liberacaoCors(response);
